...
class IAF_DDSF(BaseFlow):
    ...
    def __init__(self, dim, hid_dim, context_dim, num_layers,
        ...
        self.num_ds_dim = int(num_ds_dim)
        self.num_ds_layers = int(num_ds_layers)

        if type(dim) is int:
            self.mdl = iaf_modules.cMADE(
                    self.dim, int(hid_dim), self.context_dim, int(num_layers), 
        ...
        for i in range(num_ds_layers):
            ...
            self.add_module('sf{}'.format(i),
                            DenseSigmoidFlow(in_dim,
                                             ...
                                             out_dim))
        if type(dim) is int:
            in_ = int(num_ds_multiplier*(hid_dim/dim)*num_ds_layers)
            self.out_to_dsparams = nn.Conv1d(
                in_, 
                num_dsparams, 1)
        else:
            in_ = num_ds_multiplier*(hid_dim/dim[0])*num_ds_layers
            self.out_to_dsparams = nn.Conv1d(
                in_, 
                num_dsparams, 1)
