...
class WNlinear(Module):
    ...
    def __init__(self, in_features, out_features, 
        ...
        self.norm = norm
        self.direction = Parameter(torch.Tensor(self.out_features, self.in_features))
        self.scale = Parameter(torch.Tensor(self.out_features))
        if bias:
...
class CWNlinear(Module):
    ...
    def __init__(self, in_features, out_features, context_features,
        ...
        self.norm = norm
        self.direction = Parameter(torch.Tensor(self.out_features, self.in_features))
        self.cscale = nn.Linear(self.context_features, self.out_features)
        self.cbias = nn.Linear(self.context_features, self.out_features)
        self.reset_parameters()
